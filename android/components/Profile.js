...
export default class Profile extends React.Component {
    ...
    fetch('http://ec2-13-59-228-147.us-east-2.compute.amazonaws.com:8080/edit-profile', {
      ...
      .then(response => {
        response
          ...
          .catch(error => {
            console.log('error modifying profile', error);
          });
      })
      .catch(error => {
  ...
  renderItem(item) {
    ...
    return (
      <Swipeout style={styles.swipe} right={swipeBtns} autoClose={true}>
        <View>
  ...
  render() {
    ...
    return (
      <ScrollView>
        <CardView cardElevation={5} cardMaxElevation={0} cornerRadius={0}>
          {state.params.avatar ? (
            <View>
              <View style={styles.bgContainer}>
                <Image source={{ uri: state.params.avatar }} style={styles.avatarBG} blurRadius={5}>
                  <Text style={styles.userInfo}>{`${state.params.first_name} ${state.params.last_name}`}</Text>
                </Image>
              </View>
              <View style={styles.overlay}>
                <Image source={{ uri: state.params.avatar }} style={styles.avatar} />
              </View>
            </View>
          ) : (
            <View>
              ...
              <View style={styles.overlay}>
                <Image source={require('../app/src/user_icon.png')} style={styles.avatar2} />
              </View>
            </View>
          )}
        </CardView>
        <View style={styles.avoidableList}>
          ...
          <View style={{ marginTop: 5 }}>
            <CardView cardElevation={2} cardMaxElevation={1} cornerRadius={10}>
